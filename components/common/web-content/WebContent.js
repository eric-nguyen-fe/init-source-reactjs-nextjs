...
class WebContent extends Component {
    ...
    render() {
        ...
        switch (type) {
                    ...
                    <div className={classes.page_header}>
                        {contents?.poweredBy &&
                            <div className={classes.logo_view}>
                                ...
                                <span className={classes.logo_text}>{i18nText('intro.poweredBy')}</span>
                            </div>
                        }
                        {/* {contents?.lngSwitcher && <LanguageSwitcher offset={8} />} */}
                    </div>
                );
            }
            case 'navBackBtn': {
            ...
            case 'privacyPolicy': case 'termsOfService': {
                if (!isArray(contents)) { return null; }
                return (
                    <Paper className={classes.pp_tos_view}>
                        {contents.map((content, index) => {
                            const { type: contentType, text, desc, padding } = content;
...
WebContent.propTypes = {
    type: PropTypes.oneOf(['backdrop', 'header', 'navBackBtn', 'intro', 'wellcome', 'footer', 'privacyPolicy', 'termsOfService']),
